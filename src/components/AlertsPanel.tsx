              ...
              {payload.events.slice(0, 8).map((event) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <p className="min-w-0 break-words text-sm font-medium tracking-[-0.02em]">
                      {event.title}
                    </p>
                    {!event.readAt ? (
                  ...
                  <p className="mt-1 break-words text-xs leading-relaxed text-[var(--muted-strong)]">
                    {event.body}
                  </p>
                </li>
              ))}
          ...
          {payload && payload.rules.length > 0 ? (
            <ul className="mt-4 space-y-2">
              {payload.rules.map((rule) => (
                <li
                  key={rule.id}
                  className="flex flex-col items-stretch gap-3 rounded-[12px] border border-[var(--line)] px-3 py-2 sm:flex-row sm:items-center sm:justify-between"
